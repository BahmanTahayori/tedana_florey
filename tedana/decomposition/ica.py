...
def tedica(data, n_components, fixed_seed, ica_method=DEFAULT_ICA_METHOD, n_robust_runs=DEFAULT_N_ROBUST_RUNS, maxit=DEFAULT_N_MAX_ITER, maxrestart=DEFAULT_N_MAX_RESTART):
    """
    ...
    if ica_method=='robustica':
        mmix, fixed_seed = r_ica(data, n_components = n_components, fixed_seed=fixed_seed, n_robust_runs=n_robust_runs, max_it=maxit)
    elif ica_method=='fastica':
        mmix, fixed_seed=f_ica(data, n_components =n_components, fixed_seed=fixed_seed, maxit=maxit, maxrestart=maxrestart)
    else:
...
def r_ica(data, n_components, fixed_seed, n_robust_runs, max_it):
    ...
    if n_robust_runs>200:
        LGR.warning("The selected n_robust_runs is a very big number! The process will take a long time!")
    ...
    try:
       rica = RobustICA(n_components=n_components, robust_runs=n_robust_runs, whiten='arbitrary-variance',max_iter= max_it,
                     random_state=fixed_seed, robust_dimreduce=False, fun='logcosh', robust_method='DBSCAN')

       S, mmix = rica.fit_transform(data)
       q = rica.evaluate_clustering(rica.S_all, rica.clustering.labels_, rica.signs_, rica.orientation_)

    except:
       rica = RobustICA(n_components=n_components, robust_runs=n_robust_runs, whiten='arbitrary-variance', max_iter=max_it,
                        random_state=fixed_seed, robust_dimreduce=False, fun='logcosh', robust_method='AgglomerativeClustering')
    ...
    if Iq < 0.6:
        LGR.warning("The resultant mean Index Quality is low. It  is recommended to rerun the process with a different seed.")
